...
const entries = [
  {
    ...
    prompt:
      "Vertical 4:5 LinkedIn key art, cinematic Nordic autumn city, existing mobility app remains primary, subtle ParkKey reward layer after verified event, canonical Parky exactly as approved, premium realistic photography.",
  ...
  },
  {
    ...
    prompt:
      "Vertical 4:5 cinematic Nordic autumn street, authentic reward moment, canonical Parky exactly as approved, premium realistic photography.",
  ...
  },
  {
    ...
    prompt:
      "Vertical 4:5 realistic Nordic parking-to-city flow, generic parking app, verified event to reward to next behaviour, canonical Parky exact reference.",
  ...
  },
  {
    ...
    prompt:
      "Vertical 4:5 cinematic Nordic town centre, local independent commerce, subtle ParkKey reward, no invented merchant logo, canonical Parky.",
  ...
  },
  {
    ...
    prompt:
      "Vertical 4:5 Nordic mobility scene with restrained CoreOS proof layer; event verification and KPI framework without fabricated values.",
  ...
  },
  {
    ...
    prompt:
      "Vertical 4:5 cinematic Nordic autumn city with several natural mobility choices, canonical Parky and ParkKey, one clear pilot CTA.",
 ...
 */
export async function ensureLinkedInAutumnSeries2026(
  ...
  const assetRows = missing.map((entry) => ({
    ...
    source_notes:
      "Approved ParkKey logo and canonical Parky only; no unverified partner logos or claims.",
  ...
  const postRows = missing.map((entry) => ({
    ...
    negative_prompt:
      "No fake partner logos, no redesigned Parky, no fabricated statistics, no fake provider evidence.",
    alt_text: entry.alt,
  ...
  const schedules = await db.from("social_schedules").insert(
    missing.map((entry) => ({
      ...
      status: SCHEDULE_STATUS,
      notes:
        "Editorial schedule only. Never mark PUBLISHED without verified LinkedIn provider post ID/URL.",
      created_by: userId,
    })),
  );
  if (schedules.error && schedules.error.code !== "23505") {
