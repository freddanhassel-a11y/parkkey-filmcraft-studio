...
export const Route = createFileRoute("/_authenticated/studio/delivery")({
  head: () => ({
    meta: [
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-3 md:grid-cols-3">
        <ProofCard title="1. CoreOS-kontakt" text="Mottagaren måste matcha en verklig CoreOS-kontakt." />
        <ProofCard title="2. Godkänt material" text="Backend kräver APPROVED eller EXPORTED source-of-truth." />
        <ProofCard title="3. Explicit bekräftelse" text="Slutläget blir READY TO SEND IN COREOS, aldrig fejkad SENT." />
      </div>
      ...
      <section className="space-y-4 rounded-2xl border border-border bg-card/70 p-6">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <Field label="Pakettitel" id="delivery-title">
            <Input id="delivery-title" value={draft.title} onChange={(event) => setDraft({ ...draft, title: event.target.value })} />
          </Field>
          <Field label="Sanningsstatus" id="delivery-truth">
            <Select value={draft.truth_label} onValueChange={(value) => setDraft({ ...draft, truth_label: value })}>
              <SelectTrigger id="delivery-truth"><SelectValue /></SelectTrigger>
              <SelectContent>
          ...
          <Field label="Ämne" id="delivery-subject">
            <Input id="delivery-subject" value={draft.subject} onChange={(event) => setDraft({ ...draft, subject: event.target.value })} />
          </Field>
          <Field label="Giltigt till" id="delivery-expiry">
            <Input id="delivery-expiry" type="date" value={draft.expires_at} onChange={(event) => setDraft({ ...draft, expires_at: event.target.value })} />
          </Field>
          <Field label="Godkänt filmprojekt" id="delivery-project">
            <Select value={draft.film_project_id || "none"} onValueChange={(value) => setDraft({ ...draft, film_project_id: value === "none" ? "" : value })}>
              <SelectTrigger id="delivery-project"><SelectValue placeholder="Valfritt" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">Inget filmprojekt</SelectItem>
                {approvedProjects.map((project) => <SelectItem key={project.id} value={project.id}>{project.title} · {project.status}</SelectItem>)}
              </SelectContent>
            </Select>
          </Field>
          <Field label="Godkänt media" id="delivery-asset">
            <Select value={draft.media_asset_id || "none"} onValueChange={(value) => setDraft({ ...draft, media_asset_id: value === "none" ? "" : value })}>
              <SelectTrigger id="delivery-asset"><SelectValue placeholder="Valfritt" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">Ingen mediafil</SelectItem>
                {approvedAssets.map((asset) => <SelectItem key={asset.id} value={asset.id}>{asset.name} · {asset.approval_status}</SelectItem>)}
              </SelectContent>
            </Select>
          </Field>
          <Field label="Godkänt socialt paket" id="delivery-post">
            <Select value={draft.social_post_id || "none"} onValueChange={(value) => setDraft({ ...draft, social_post_id: value === "none" ? "" : value })}>
              <SelectTrigger id="delivery-post"><SelectValue placeholder="Valfritt" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">Inget socialt paket</SelectItem>
                {approvedPosts.map((post) => <SelectItem key={post.id} value={post.id}>{post.title} · {post.status}</SelectItem>)}
              </SelectContent>
            </Select>
          </Field>
          <div className="space-y-1.5 md:col-span-2">
            <Label htmlFor="delivery-message">Meddelande</Label>
            <Textarea id="delivery-message" value={draft.message} onChange={(event) => setDraft({ ...draft, message: event.target.value })} />
          </div>
        </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Detta skickar fortfarande inget mail. Bekräftelsen flyttar paketet till READY TO SEND IN COREOS och försöker logga leveransen på den kopplade CoreOS-posten.
              </p>
            </div>
          </div>
          <div className="flex items-start gap-2">
            <Checkbox id="confirm-delivery" checked={confirmChecked} onCheckedChange={(value) => setConfirmChecked(value === true)} />
      ...
      <section className="space-y-3">
        ...
        {delivery.isLoading ? <p className="text-sm text-muted-foreground">Hämtar leveranser…</p> : null}
        {delivery.error ? <p className="text-sm text-status-error">Kunde inte hämta leveranser: {delivery.error.message}</p> : null}
        <ul className="space-y-3">
          {(delivery.data?.packages ?? []).map((pkg) => {
            const recipients = (delivery.data?.recipients ?? []).filter((recipient) => recipient.package_id === pkg.id);
            return (
              <li key={pkg.id} className="rounded-xl border border-border bg-card/60 p-4">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {pkg.coreos_display_name ?? "Ingen CoreOS-post"} · {recipients.map((recipient) => recipient.email).join(", ") || "Ingen mottagare"}
                    </p>
                  </div>
